# magcache_nodes.py (完成版)
#
# 目的：
# - MagCacheApplyクラスに完全なキャッシュ適用ロジックを実装する。
# - キャリブレーションと適用の両方がOmniGen2モデルで正しく機能するようにする。
...
# --- コア機能とヘルパーをインポート ---
# interpolate_mag_ratios と MagCacheState もインポートする
...
def universal_magcache_forward(self, *args, **kwargs):
    ...
    state = state_manager.get_state(cache_key) # State manager handles unknown keys
    
    skip_this_step = False
    if current_step >= params['start_step_abs'] and current_step < params['end_step_abs']:
        # キャリブレーションデータは1種類しかないので、呼び出しの種類によらず同じmag_ratiosを使う
    ...
    if skip_this_step:
        # print(f"[MagCache] Step {current_step}: Cache HIT for '{cache_key}'")
...
class MagCacheApply(object):
    @classmethod
    def INPUT_TYPES(s): return { "required": { "model": ("MODEL",), "enabled": ("BOOLEAN", {"default": True}), "magcache_thresh": ("FLOAT", {"default": 0.1, "min": 0.0, "max": 1.0, "step": 0.01}), "magcache_k": ("INT", {"default": 2, "min": 1, "max": 10, "step": 1}), "retention_ratio": ("FLOAT", {"default": 0.2, "min": 0.0, "max": 1.0, "step": 0.01}), "exclude_last_step": ("BOOLEAN", {"default": True}), } }
    RETURN_TYPES = ("MODEL",)
    ...
    def apply_magcache(self, model, enabled, magcache_thresh, magcache_k, retention_ratio, exclude_last_step):
        ...
        model_name = _get_model_name(model)
        if not model_name: return (new_model,)
        
        model_hash = get_model_hash(model_name)
        mag_ratios = load_mag_ratios(model_hash)
        if mag_ratios is None:
            print(f"[MagCache Apply] Warning: Calibration data for '{model_name}' not found. MagCache is disabled.")
            return (new_model,)
        ...
        diffusion_model.magcache_is_calibrating = False # 適用モードであることを示す
        ...
        new_model.set_model_unet_function_wrapper(unet_wrapper_function)
        print("[MagCache Apply] Enabled and patched.")
        return (new_model,)
